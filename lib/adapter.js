...
module.exports = (function () {
  ...
  var adapter = {
    ...
    describe: function (connection, collection, cb) {
      ...
      /*var statement = "" +
        "SELECT " +
          ...
          "ISNULL(" +
            "(SELECT " +
              "is_unique FROM  master.dbo.sysindexes i" +
            "LEFT OUTER JOIN sys.sysindexes ic ON i.index_id=ic.index_id " +
            "WHERE i.object_id=t.object_id AND ic.object_id=t.object_id AND ic.column_id=c.column_id" +
            "),0) AS [Unique]," +
          "ISNULL(" +
            "(SELECT " +
              "is_primary_key " +
            "FROM  master.dbo.sysindexes i " +
            "LEFT OUTER JOIN  master.dbo.sysindexes ic ON i.index_id=ic.index_id " +
            "WHERE i.object_id=t.object_id AND ic.object_id=t.object_id AND ic.column_id=c.column_id" +
            "),0) AS PrimaryKey," +
          "ISNULL(" +
          "(SELECT" +
        ...
        "LEFT OUTER JOIN sys.index_columns ic ON ic.object_id=t.object_id WHERE t.name='" + tableName + "'";*/
      var statement = "select c.name AS ColumnName," +
        "TYPE_NAME(c.xtype) AS TypeName," +
        "c.isnullable as Nullable," +
        "CASE WHEN autoval IS NOT NULL THEN 1 ELSE 0 END as AutoIncrement," +
        "CASE WHEN autoval IS NOT NULL THEN 1 ELSE 0 END as [Unique]," +
        "CASE WHEN autoval IS NOT NULL THEN 1 ELSE 0 END as PrimaryKey " +
        ",ISNULL( (select " +
        "	COUNT(*) " +
        "	FROM  sysindexes i " +
        "	LEFT OUTER JOIN  sysindexkeys ic ON i.id=ic.id " +
        "	WHERE i.id=t.id AND ic.id=t.id AND ic.colid=c.colid " +
        "),0) AS Indexed " +
        "from SYSOBJECTS t " +
        "INNER JOIN syscolumns c ON (c.id=t.id)AND(t.xtype = 'U') "+
        "LEFT OUTER JOIN sysindexkeys ic ON ic.id=t.id WHERE t.name='" + tableName + "' " +
        "";
    ...
    drop: function (connection, collection, relations, cb) {
      // Add in logic here to delete a collection (e.g. DROP TABLE logic)  dbo.
      var statement = 'IF OBJECT_ID(\''+ collection + '\', \'U\') IS NOT NULL DROP TABLE ' + collection + '';
